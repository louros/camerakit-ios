...
@objc open class RecordButton : UIButton {
    ...
    open var buttonColor: UIColor! = .blue{
        didSet {
            ...
            circleBorder.borderColor = buttonColor.cgColor.copy(alpha: 0.5)
        }
    }
    open var progressColor: UIColor!  = .red {
        didSet {
            maskLayer.backgroundColor = progressColor.cgColor
        }
    }
    
    open var pictureMode: Bool = true
    ...
    fileprivate func drawButton() {
        ...
        let size: CGFloat = self.frame.size.width - 16
        ...
        maskLayer = CALayer()
        maskLayer.frame = CGRect(x: 5, y: 5, width: self.frame.width, height: self.frame.height)
        maskLayer.backgroundColor = progressColor.cgColor
        
        progressLayer = CAShapeLayer()
        progressLayer.path = UIBezierPath(arcCenter: CGPoint(x: self.bounds.midX,y: self.bounds.midY), radius: self.frame.size.width / 2 - 2, startAngle: startAngle, endAngle: endAngle, clockwise: true).cgPath
